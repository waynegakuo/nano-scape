...
export class Weather implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    if (selectedImage && selectedImage.prompt.concept === 'Weather') {
      ...
      if (this.scapeGenerator && this.scapeGenerator.form) {
        this.scapeGenerator.form.patchValue({ city: selectedImage.prompt.data.city });
      }
  ...
  async generateImage(formValue: any) {
    ...
    const userPrompt: UserPrompt = { concept: 'Weather', data: cityInfo };
