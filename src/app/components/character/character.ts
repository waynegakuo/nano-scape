import { Component, inject, signal, OnDestroy, ViewChild, OnInit, AfterViewInit } from '@angular/core';
...
export class Character implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    if (selectedImage && selectedImage.prompt.concept === 'Character') {
      ...
      if (this.scapeGenerator && this.scapeGenerator.form) {
        this.scapeGenerator.form.patchValue({ character: selectedImage.prompt.data.characterName });
      }
  ...
  async generateImage(formValue: any) {
    ...
    const userPrompt: UserPrompt = { concept: 'Character', data: characterInfo };
